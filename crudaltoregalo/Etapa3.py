...
class Inventario:
    ...
    def modificar_producto(self, codigo, nueva_descripcion, nueva_cantidad, nuevo_precio):
        ...
        if producto:
            ...
            self.conexion.commit()
            return jsonify({'message': 'Recordatorio modificado correctamente.'}), 200
        return jsonify({'message': 'Recordaorio no encontrado.'}), 404
    ...
    def eliminar_producto(self, codigo):
        ...
        if self.cursor.rowcount > 0:
            self.conexion.commit()
            return jsonify({'message': 'Recordatorio eliminado correctamente.'}), 200
        ...
        for item in self.items:
            if item.codigo == codigo:
                ...
                self.cursor.execute("UPDATE productos SET cantidad = cantidad - ? WHERE codigo = ?",
                                    (cantidad, codigo))
                self.conexion.commit()
                return jsonify({'message': 'Recordatorio agregado al carrito correctamente.'}), 200
